...
//		(rp == null) ? "": rp.getNom();
		...
		out.println("<div style='background-color: darkgrey; height:100%;'><form method='post' action='ControleGestionResident'>"
		...
		if(disable) {
			out.println("<div class=\"mainBlock\" style='justify-content: center; align-items:center;'>"
					...
					+ "</div>"
					+ "<input type='text' name='texture' value="+rp.getNomRegime()+" disabled='disabled'>"
					);
		}else {
			out.println("<br><div class=\"mainBlock\">\r\n" + "<div></div>" + "        <br>Pathologie  <div class=\"left\" id=\"D\" style=\"visibility:"+ (disable? "hidden":"visible") +";\">   \r\n"
					...
					+ "            \r\n" + "        </div>\r\n"
					+ "		<select id=\"listOut0\" name=\"listOut0\" style='visibility:hidden;' multiple></select>	"
					+ "<div></div>" + "    </div>");
			...
			out.println("<br><div class=\"mainBlock\">\r\n" + "<div></div>" + "        <br>Allergie<div class=\"left\" id=\"D2\">   \r\n"
					...
					+ "            \r\n" + "        </div>\r\n"
					+ "		<select id=\"listOut1\" name=\"listOut1\" style='visibility:hidden;' multiple></select>	"
					+ "<div></div>" + "    </div>");
			
			out.println("<select name='texture' id='texture'>");
			ArrayList<String> listTex = new ArrayList<>();
			listTex=Utils.getList("regime", session);
			
				for (int i=0 ; i<listTex.size() ; i++) {
					
					out.println("<option value='"+listTex.get(i)+"'" +(rp!=null ? (rp.getNomRegime().equals(listTex.get(i))  ? "selected" : "") : "")+">"+listTex.get(i)+"</option>");
				}
