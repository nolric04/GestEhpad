...
public class ViewMultiRegistre extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < 2; i++) {
			...
			if(two) {
				response.getWriter().append("<span> <h1 style='color:white;'>Global</h1> </span>");
				two = false;
			}else if(!App.isAdmin() && !two) {
				response.getWriter().append("<span> <h1 style='color:white;'>"+ p.getService() +"</h1> </span>");
			}
		...
		if (App.isAdmin()) {
			response.getWriter().append("<div class=\"reg3\"><span> <h1 style='color:white;'>Admin</h1> </span>");
