...
public class ControleEvenement extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String r = (!request.getParameter("idEvent").isEmpty() ? request.getParameter("idEvent") : "");
		int reg = (!r.isBlank() ? Integer.parseInt(r) : null);
		
		// parcours de la collection registre;
		for ( int i = 0 ; i < regi.getListEvenements().size() ; i++ ) 
		{
		...
		if ( str.equals("Modifier") ) {
			...
			//session.setAttribute("evenement", regi.getListEvenements().get(0) );
