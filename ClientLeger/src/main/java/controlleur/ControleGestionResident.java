...
public class ControleGestionResident extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Resident working_resident = (Resident) session.getAttribute("res");
		System.out.println(working_resident.getNom());
		DaoAccess bdd = (DaoAccess) session.getAttribute("BDD");
		Etablissement etab = (Etablissement) session.getAttribute("etablissement");
		
		String prenomRes = (String) request.getParameter("prenomRes");
		String nomRes = (String) request.getParameter("nomRes");
		String numSSRes = (String) request.getParameter("numSecu");
		String numChambreRes = (String) request.getParameter("numChambre");
		String nomContact = (String) request.getParameter("nomParent");
		String numTelContact = (String) request.getParameter("numTel");
		String lienContact = (String) request.getParameter("lienParent");
		String dateNaissance = (String) request.getParameter("dateNaissance");
		String texture = (String) request.getParameter("texture");
		...
		try {
			...
			for (int i=0 ; i<str.length ; i++) {
				try {
					...
					bdd.getPreparedStatement().setInt(1, tmp );
			...
			for (int i=0 ; i<str.length ; i++) {
				try {
				String sql = "SELECT * FROM allergie WHERE idAllergie = ?;";
				bdd.setPreparedStatement(sql);
				bdd.getPreparedStatement().setInt(1, Integer.parseInt(str[i]));
				ResultSet rsUsers = bdd.getPreparedStatement().executeQuery();
				while (rsUsers.next())
					lsAllergie.add( rsUsers.getString("nomAllergie") );
				}
				catch(Exception e) {
					e.printStackTrace();
