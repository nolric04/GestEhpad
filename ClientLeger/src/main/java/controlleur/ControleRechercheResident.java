...
public class ControleRechercheResident extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!txtNom.isBlank() || !txtPrenom.isBlank() || !txtNumSecu.isBlank() || !numChambre.isBlank())
		{
			...
			{
				if(( !txtNom.isBlank() && re.getNom().toLowerCase().contains(txtNom.toLowerCase()))
					|| (!txtPrenom.isBlank() && re.getPrenom().toLowerCase().contains(txtPrenom.toLowerCase()))
					|| (!txtNumSecu.isBlank() && re.getNumSecu().toLowerCase().contains(txtNumSecu.toLowerCase()))
