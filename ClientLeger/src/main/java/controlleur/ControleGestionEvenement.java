...
public class ControleGestionEvenement extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HttpSession session=request.getSession();
			...
			//Evenement event = (Evenement) (session.getAttribute("evenement") instanceof Evenement ? session.getAttribute("evenement") : null);
...
//			out.println(event.getDescription()+event.getDateEmission());
			...
			String res=(String) request.getParameter("listOut");
		...
		try 
		{
			...
			if(!qqc[0].isEmpty())
			{
					for(int i=0; i<qqc.length;i++)
				{
					 ...
					 System.out.println(listRes.get(i).getNom());
				}
			}
